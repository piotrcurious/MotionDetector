...
// Optimized for production readiness on ESP32
...
struct Kalman2D {
    ...
    float update(float z, float dt) {
        ...
        p00 = p00_u; p01 = p01_u; p10 = p10_u; p11 = p11_u;
...
static void wifi_promiscuous_enhanced_cb(void* buf, wifi_promiscuous_pkt_type_t type) {
    ...
    if (payload == NULL || pkt->rx_ctrl.sig_len < 24) return;
    ...
    float dt = 0.25f;
    if (bssidBanks[bank].last_seen_ms != 0) {
        dt = (now - bssidBanks[bank].last_seen_ms) * 0.001f;
    }
    if (dt <= 0) dt = 0.05f;
    if (dt > 5.0f) dt = 5.0f; // Cap dt for stability
...
void motionDetector_radio_update_from_probe(const uint8_t* mac, int rssi, int channel) {
    if (mac == NULL) return;
    if (rssi < -120 || rssi > 0) return;
    
    int bank = allocate_bank_for_mac(mac, channel);
    if (bank < 0) return;
    ...
    bssidBanks[bank].last_seen_ms = now;
    bssidBanks[bank].last_rssi = rssi;
    bssidBanks[bank].channel = channel;
    bssidBanks[bank].age_counter = ++global_age_counter;

    float filtered = bssidBanks[bank].kf.update((float) rssi, dt);
    update_channel_kf(channel, filtered, dt);
}
